...
def export_individual_metric_charts(summary_df, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_01_cumulative_pnl.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_02_pnl_per_match.png"))
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_03_win_rate.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_04_traded_volume.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_05_peak_inventory.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_06_hedge_sweeps.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_07_hedge_cost.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_08_pnl_distribution.png"))
    plt.close()
    ...
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "metric_09_drawdown.png"))
    plt.close()

def export_gueant_charts(summary_df, sample_history=None, sample_match_name="", output_dir="result"):
    """
    ...
    export_individual_metric_charts(summary_df, output_dir)

    # 1. Save detailed CSV summary
    csv_file = os.path.join(output_dir, "gueant_backtest_summary.csv")
    ...
    fig.suptitle("GUEANT-CARTEA RISK-CONTROLLED MARKET MAKER - BACKTEST DASHBOARD ($50k CAP, AUTO-HEDGE)",
    ...
    dashboard_path = os.path.join(output_dir, "gueant_backtest_dashboard.png")
    ...
    if sample_history is not None and not sample_history.empty:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        micro_path = os.path.join(output_dir, "gueant_microstructure.png")
...
if __name__ == "__main__":
    ...
    for league, files in league_files.items():
        print(f"\n{'='*90}")
        print(f"PROCESSING LEAGUE: {league.upper()} ({len(files)} matches)")
        print(f"{'='*90}")
        ...
        for idx, csv_file in enumerate(files):
            try:
                ...
                if res:
                    ...
                    print(f"[{idx+1}/{len(files)}] Finished: {res['Match']} | "
        ...
        if league_results:
            ...
            league_out_dir = os.path.join("result", league)
            print(f"\n[+] Exporting charts for {league.upper()} to '{league_out_dir}'...")
            export_gueant_charts(league_summary_df, sample_history=sample_history,
                                 sample_match_name=sample_match_name, output_dir=league_out_dir)

    if all_results:
        summary_df = pd.DataFrame(all_results)
        print("\n" + "=" * 90)
        print("GUEANT-CARTEA RISK-CONTROLLED BACKTEST SUMMARY (ALL LEAGUES):")
        print("=" * 90)
        ...
        print("\n" + "=" * 90)
        print("QUANTITATIVE RISK & PERFORMANCE METRICS (GUEANT-CARTEA RISK-CONTROLLED MODEL - TOTALS):")
        print("=" * 90)
        ...
        # Export TOTALS charts and summary reports to result/totals folder
        print("\n" + "=" * 90)
        print("EXPORTING TOTALS CHARTS AND REPORTS TO 'result/totals/' DIRECTORY...")
        print("=" * 90)
        totals_out_dir = os.path.join("result", "totals")
        export_gueant_charts(summary_df, sample_history=None,
                             sample_match_name="ALL LEAGUES AGGREGATED", output_dir=totals_out_dir)
        print("\n[V] Gueant-Cartea TOTALS chart and report export completed successfully!")
